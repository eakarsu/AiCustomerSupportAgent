...
/**
 * Gap/CFS Feature Page: Agent performance prediction
 * Slug: agent-performance-prediction
 * Backend: /api/cf-agent-performance-prediction
 */
export default function AgentPerformancePredictionPage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const res = await fetch('/api/cf-agent-performance-prediction', {
        method: 'POST',
        headers: {
  ...
  return (
    <div style={{ padding: 24, maxWidth: 900, margin: '0 auto', color: '#e5e7eb' }}>
      <h2 style={{ marginBottom: 8 }}>Agent performance prediction</h2>
      <p style={{ opacity: 0.75, marginBottom: 16 }}>ML model identifies which agents are best at customer retention, upselling; recommend specialized skill training.</p>
